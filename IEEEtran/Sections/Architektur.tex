...
\subsubsection{Konsumentensichtweise}
...
\begin{figure}[h]
	...
	\label{ConsumerView}
\end{figure}
Alle Kategorien von Cloud-Computing können sogenannte Sicherheitsperimeter verwenden. Diese regeln den Zugriff auf die Ressourcen \cite{Badger}.
...
Bei der lokalen Community Cloud teilen mehrere Organisationen ihre Ressourcen und Daten. Alle Organisationen stellen und/oder konsumieren Cloud Dienste.
Dabei muss mindestens eine Organisation solche Dienste zur Verfügung stellen, sprich Provider sein.
...
Außerdem müssen die Kommunikationskanäle geschützt und bei der Verwendung des öffentlichen Internets, Kryptographie verwenden werden. Die einzelnen Kommunikationskanäle zwischen den Teilnehmern 
...
hierbei von den Sicherheitsperimetern der einzelnen Organisationen ab. Ein großer Nachteil dieser Methode sind die hohen Kosten der einzelnen Provider, die Ressourcen 
anschaffen und Dienste konfigurieren müssen. Die Konsumenten hingegen zahlen nur die Mietkosten für die Verwendungen der einzelnen Clouds. Der Nachteil ist hier analog zur lokalen privaten Cloud \cite{Badger}.
...
Hier kann der Konsument einen eigenen Sicherheitsperimeter installieren, um den Zugriff zur Cloud zu regeln.
...
von dem Internetprovider, dem DNS Server und der Routerinfrastruktur abhängt. Die Arbeitslast oder Ressourcen eines Konsumenten können zu jeder Zeit
vom Provider migriert werden. Ein großer Vorteil der Public Cloud ist die Kosteneffizienz, da Datenzentren an Standorten verwendet werden können, die für den Konsumenten am günstigsten sind.
Ein Beispiel dafür wäre, dass die Arbeitslast eines deutschen Unternehmens in einem deutschen Datenzentrum verarbeitet und somit die Performance verbessert wird (kurze Übertragungswege).
...
Außerdem haben die Konsumenten keine Möglichkeit die Einsicht ihrer Daten zu überwachen oder eine Authentifizierung für ihre Daten einzurichten. Da große Anbieter meist ein Monitoring für die
...
Bei Software as a Service hat der Konsument nur die Kontrolle auf der Benutzerebene, d.h. dieser kann Benutzer verwalten, wie z.B. Benutzer hinzufügen.
Der Provider hingegen hat die totale Kontrolle über alle Bereiche. Dieser ist verantwortlich für das Erstellen, Konfigurieren und Verwalten der Anwendungen, 
sodass der Konsument diesen Dienst nutzen kann.
...
Die Vorteile solcher Dienste sind, dass für öffentliche und ausgelagerten Szenarien, sich der Großteil der von einer Anwendung verwalteten Daten auf den Softwareverteilungslösungen
...
\begin{figure}[h]
	...
	\label{SaaSM2}
\end{figure}
Beim statischen Szenario (siehe \textbf{Abbildung \ref{SaaSM2}}) wird die Anwendung vom Provider so implementiert, dass mehrere Klienten gleichzeitig daran arbeiten können und die Daten in nur einem Datenbanksystem gespeichert werden. 
Dies hat den Vorteil, dass die Kosten für den Provider gesenkt werden, da nur noch eine Anwendung betrieben wird.
...
Wobei bei SaaS der Konsument und seine Klienten von der Anwendung des Providers abhängig sind. Dies verschafft den Konsumenten eine deutlich erhöhte Sicherheit über den Ablauf
der Anwendung und seiner Daten.
Der Provider kümmert sich weiterhin um ein stabiles und sicheres Netzwerk und verspricht eine hohe Performance von verschiedenen Standorten der Welt.
Außerdem sind auch hier die Ressourcen skalierbar, sodass dem Konsumenten Flexibilität ermöglicht wird. 
...
Zusätzlich kümmert er sich um die Top-Level Ressourcen Verwaltung, wie z.B. ob die Cloud genug Ressourcen für die Anfrage hat und welcher Cluster Manager die benötigten freien Ressourcen besitzt.
...
Ein Cluster Manager ist für die Operationen von mehreren Computern verantwortlich, die mit einem High-Speed LAN verbunden sind. Ein solches Computercluster kann mehrere tausend Computer beinhalten, die geographisch voneinander getrennt sein können.
...
Auf Anfrage des Cluster Managers antwortet der Computer Manager auf Statusanfragen. Ein Computer Manager verwendet die Befehlsschnittstelle seines Hypervisors, um virtuelle Maschinen anzulegen, löschen, rekonfigurieren und Einstellungen des virtuellen Netzwerks einzustellen.
Der Computer Manager ist außerdem verantwortlich für die Optimierung der Ressourcen. Da verschiedene Daten von verschiedenen Konsumenten auf einem Computer laufen können, muss der Hypervisor sich darum kümmern, das es keine Datenvermischung gibt.
Die Arbeitslast eines Konsumenten kann dabei auch von mehreren Computern, sowie Clustern verarbeitet werden. 
Eine Methode für das Ressourcenmanagement, ist das Verteilen der Arbeitslast von gering ausgelasteten Maschinen auf bereits hoch ausgelastete Maschinen. Dies hat den Vorteil, dass einzelne Maschinen abgeschaltet werden können, um Ressourcen einzusparen oder Wartungsarbeiten auf einzelnen Maschinen durchzuführen.
